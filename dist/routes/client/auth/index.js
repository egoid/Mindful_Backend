...
var employee_routes = require('./employee');
var employer_routes = require('./employer');
...
var USER_FIELDS = ["user_id", "alias", "user_type", "email", "verified", "user_role_id", "facebook_id", "linkedin_id", "is_deleted", "employer_id", "employee_id"];
...
function require_session_key(req, res, next) {
  ...
  if (!session_key) {
    ...
    var sql = "SELECT user.*, employer.employer_id, employee.employee_id " + "FROM user " + "JOIN user_session USING(user_id) " + "LEFT JOIN employee USING(user_id) " + "LEFT JOIN employer USING(user_id) " + "WHERE user_session_key = ?";

    var values = [session_key];
