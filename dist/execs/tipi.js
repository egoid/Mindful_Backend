...
var ipip_algorithm = module.exports = function (ans) {
  var num_arr = [6, 7, 8, 9, 10, 15, 16, 17, 18, 19, 20];
  var trait_range = ["low", "avg", "high"];
  ...
  var ipip_scores = {};
  var trait_keys = Object.keys(narrative);
  var trait_keys_iterator = trait_keys.entries();
  ...
  try {
    for (var _iterator = trait_keys_iterator[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var item = _step.value;

      var indx = item[0],
          _trait = item[1];
      ipip_scores[_trait] = ans[indx + 1] + ans[indx + 6] + ans[indx + 11] + ans[indx + 16];
    }
  } catch (err) {
  ...
  var output = {};
  for (var trait in ipip_scores) {
    for (var idx in ["low", "avg", "high"]) {
      if (ipip_scores[trait] >= ipip_ranges[trait][trait_range[idx]][0] && ipip_scores[trait] <= ipip_ranges[trait][trait_range[idx]][1]) {
        output[trait] = {
